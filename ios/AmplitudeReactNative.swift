...
class ReactNative: NSObject {
  ...
  @objc
  func idfaMethod(_ name: String, callback: RCTResponseSenderBlock) {
    // NSNull()/first item is an error object (if exists)
    callback([NSNull(), ["success": true, "value": "Hello, \(name)!"]])
  }
